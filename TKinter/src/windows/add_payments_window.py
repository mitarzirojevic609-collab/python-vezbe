...
def save_payment(user_id, amout, created_at):
    connection = connect_to_db()
    cursor = connection.cursor()

    query = f"INSERT INTO payments(user_id, amount, created_at) VALUES (%s, %s, %s)"
    cursor.execute(query, (user_id.get(), amout.get(), created_at.get()))
    connection.commit()
...
def show_add_payments_window(window):
    ...
    tk.Button(window,
              text="Dodaj uplatu",
              font=("Arial", 16),
              command=lambda: save_payment(user_id, amount, created_at),
              ).pack()